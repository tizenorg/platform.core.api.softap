...
static void __softap_enabled_cfm_cb(GObject *source_object, GAsyncResult *res,
		gpointer user_data)
{
	DBG("+");
...
API int softap_enable(softap_h softap)
{
	...
	g_variant_new("(ssii)", set.ssid, set.key, set.visibility, set.sec_type),
		G_DBUS_CALL_FLAGS_NONE, -1, sa->cancellable, (GAsyncReadyCallback) __softap_enabled_cfm_cb, (gpointer)softap);
