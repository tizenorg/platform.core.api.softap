...
typedef struct {
	...
	softap_enabled_cb enabled_cb[SOFTAP_TYPE_MAX];
	void *enabled_user_data[SOFTAP_TYPE_MAX];
	softap_disabled_cb disabled_cb[SOFTAP_TYPE_MAX];
	void *disabled_user_data[SOFTAP_TYPE_MAX];
	softap_connection_state_changed_cb changed_cb[SOFTAP_TYPE_MAX];
	void *changed_user_data[SOFTAP_TYPE_MAX];
	softap_data_usage_cb data_usage_cb;
	void *data_usage_user_data;
	softap_settings_reloaded_cb ap_settings_reloaded_cb;
	void *ap_settings_reloaded_user_data;
	char *ap_ssid;
	char passphrase[SOFTAP_WIFI_KEY_MAX_LEN + 1];
	softap_security_type_e sec_type;
	bool visibility;
} __softap_h;

typedef struct {
	softap_type_e interface;			/**< interface type */
...
typedef struct {
	softap_type_e interface;			/**< interface type */
	char interface_name[SOFTAP_STR_INFO_LEN];	/**< interface alphanumeric name */
	char ip_address[SOFTPAP_STR_INFO_LEN];	/**< assigned ip addresss to interface */
	char gateway_address[SOFTAP_STR_INFO_LEN];	/**< gateway address of interface */
	char subnet_mask[SOFTAP_STR_INFO_LEN];	/**< subnet mask of interface */
} __softap_interface_t;

typedef struct {
	char ssid[SOFTAP_SSID_MAX_LEN + 1];
